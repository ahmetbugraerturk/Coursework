# Undergraduate Coursework

**Majors:** Computer Engineering & Physics 

This repository serves as a comprehensive archive of my academic journey at Koç University.

---

## Courses

Below is a curated list of my key courses, categorized by discipline. 

**Note:** Each course folder contains relevant **syllabi, lab reports, assignments, and source code**. You can navigate to the specific materials by clicking on the **Course Code**.

### Core

| Semester | Course Code | Course Name | Key Concepts | Grade |
| :--- | :--- | :--- | :--- | :---: |
| **Fall 2025** | [`ENGR200`](./ENGR200%20-%20Probability%20%26%20Random%20Variables) | **Probability & Random Variables for Engineering** | Probability Axioms, Bayes' Theorem, Random Variables (PDF/CDF/PMF), Discrete/Continuous Distributions, Central Limit Theorem | **A** |
| **Fall 2025** | [`MATH204`](./MATH204%20-%20Differential%20Equations) | **Differential Equations** | First/Second Order ODEs, Laplace Transforms, Systems of Linear Differential Equations, Heat Equations | **A** |
| **Spring 2025** | [`MATH203`](./MATH203%20-%20Multivariable%20Calculus) | **Multivariable Calculus** | Partial Derivatives, Multiple Integrals, Vector Fields, Green's & Stokes' Theorems | **A+** |
| **Fall 2024** | [`MATH106`](./MATH106%20-%20Calculus%20I) | **Calculus I** | Limits, Derivatives, Integrals, Fundamental Theorem of Calculus | **A** |
| **Summer 2024** | [`MATH107`](./MATH107%20-%20Introduction%20to%20Linear%20Algebra) | **Intro to Linear Algebra** | Vector Spaces, Eigenvalues/Eigenvectors, Linear Transformations, Orthogonality | **A+** |


### Computer Science

| Semester | Course Code | Course Name | Key Concepts and Courseworks | Grade |
| :--- | :--- | :--- | :--- | :---: |
| **Spring 2026** | [`COMP351`](./) | **Fundamentals of AI** |  | *In Progress* |
| **Spring 2026** | [`COMP304`](./) | **Operaating Systems** |  | *In Progress* |
| **Fall 2025** | [`COMP201`](./COMP201%20-%20Computer%20Systems%20and%20Programming) | **Computer Systems and Programming** | **Topics:** C, Assembly (x86-64), Memory Hierarchy <br> **Works:** Labs and Assignments | **A+** |
| **Fall 2025** | [`COMP202`](./COMP202%20-%20Data%20Structures%20and%20Algorithms) | **Data Structures & Algorithms** | **Topics:** Complexity Analysis, Sorting, Hash Tables, Trees, Graphs <br> **Works:** Small Projects | **A** |
| **Spring 2025** | [`COMP132`](./COMP132%20-%20Advanced%20Programming) | **Advanced Programming** | **Topics:** Object-Oriented Programming (Java), Inheritance, Polymorphism <br> **Works:** Labs and a [Comprehensive Game Project](./COMP132%20-%20Advanced%20Programming/Game%20Project)| **A+** |
| **Spring 2025** | [`ELEC205`](./ELEC205%20-%20Digital%20System%20Design) | **Digital System Design** | **Topics:** Boolean Algebra, K-Maps, FSM Design, Datapath & Control Units, ASM Charts <br> **Works:** Assignments | **A** |
| **Fall 2024** | [`COMP106`](./COMP106%20-%20Discrete%20Mathematics%20for%20Computer%20Science%20and%20Engineering) | **Discrete Mathematics** | Logic & Proofs, Number Theory, Recursion, Complexity Analysis, Finite Automata, Turing Machines | **A** |
| **Fall 2024** | [`COMP100`](./COMP100%20-%20Introduction%20to%20Computer%20Science%20and%20Programming) | **Intro to Computer Science** | **Topics:** Python Syntax, Recursion, Object-Oriented Concepts <br> **Works:** Labs | **A-** |


### Physics

| Semester | Course Code | Course Name | Key Concepts | Grade |
| :--- | :--- | :--- | :--- | :---: |
| **Spring 2026** | [`PHYS450`](./) | **Quantum Computing** | Superposition, Entanglement, Algorithms (Grover's, Shor's), Error Correction, Hardware (Superconducting/Trapped Ions), QML (NISQ), Qiskit/Cirq | *In Progress* |
| **Spring 2026** | [`PHYS206`](./) | **General Physics IV** | Optic, Modern Physics, Theory of Relativity, Quantum Mechanics | *In Progress* |
| **Spring 2026** | [`PHYS201`](./) | **Mechanics** |  | *In Progress* |
| **Fall 2025** | [`PHYS205`](./PHYS205%20-%20General%20Physics%20III) | **General Physics III** | **Topics:** Oscillations & Waves, Interference, Diffraction, Fourier Transform, Thermodynamics <br> **Works:** Assignments and Labs | **A** |
| **Spring 2025** | [`PHYS102`](./PHYS102%20-%20General%20Physics%20II) | **General Physics II** | Electromagnetism, Gauss's Law, Circuits, Maxwell's Equations, Electromagnetic Waves | **A+** |
| **Fall 2024** | [`PHYS101`](./PHYS101%20-%20General%20Physics%20I) | **General Physics I** | Newtonian Mechanics, Rotational Motion, Energy/Momentum Conservation, Simple Harmonic Oscillations | **A** |


### Non-Technical & Research Skills

| Semester | Course Code | Course Name | Grade |
| :--- | :--- | :--- | :---: |
| **Spring 2025** | [`ACWR106`](./ACWR106%20-%20Academic%20Writing%20for%20Science%20and%20Technology) | **Academic Writing for Sci. & Tech.** | **A** |
| **Fall 2024** | [`ACWR101`](./ACWR101%20-%20Academic%20Writing) | **Basic Academic Writing** | **B+** |

---

## Academic Integrity

The code provided in this repository is for **educational and portfolio purposes only**.
* **For Students:** Please adhere to the **Koç University Honor Code**. Do not copy or submit this code as your own work.
* **For Recruiters:** This repository is intended to demonstrate technical depth and coding style.

---
